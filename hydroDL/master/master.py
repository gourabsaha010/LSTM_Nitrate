...
def loadData(optData, TempTarget, forcing_path, attr_path, out, readX=True, readY=True ):
    if eval(optData['name']) is hydroDL.data.dbCsv.DataframeCsv:
        ...
        x = df.getDataTs(forcing_path, attr_path, out,
            ...
            doNorm=optData['doNorm'][0],
            rmNan=optData['rmNan'][0])
        y = df.getDataObs(TempTarget, forcing_path, attr_path,
...
def test(out,
    ...
    if optData['doNorm'][1] is True:
        if eval(optData['name']) is hydroDL.data.dbCsv.DataframeCsv:
            ...
            pred = hydroDL.data.camels.transNorm(pred, TempTarget, toNorm=False)
            obs = hydroDL.data.camels.transNorm(obs, TempTarget, toNorm=False)
